...
object CreditCardUtils {
    ...
    private const val CVC_LENGTH_AMERICAN_EXPRESS: Int = 4
     ...
     */
    private val PREFIXES_AMERICAN_EXPRESS: Array<String> = arrayOf("34", "37")
    ...
    private val NETWORK_RESOURCE_MAP = mapOf(
            CardNetwork.AMERICAN_EXPRESS to R.drawable.bouncer_card_amex,
    ...
    private fun isValidCardLength(cardNumber: String?, cardNetwork: CardNetwork): Boolean {
        ...
        return when (cardNetwork) {
            CardNetwork.AMERICAN_EXPRESS -> length == LENGTH_AMERICAN_EXPRESS
    ...
    private fun determineCardNetwork(cardNumber: String?, shouldNormalize: Boolean): CardNetwork {
        ...
        return when {
            hasAnyPrefix(normalizedCardNumber, *PREFIXES_AMERICAN_EXPRESS) -> CardNetwork.AMERICAN_EXPRESS
    ...
    fun isValidCVC(cvc: String?, network: CardNetwork?): Boolean {
        ...
        val validLength =
                network == null && cvcValue.length >= 3 && cvcValue.length <= 4 ||
                        CardNetwork.AMERICAN_EXPRESS == network && cvcValue.length == CVC_LENGTH_AMERICAN_EXPRESS ||
    ...
    fun formatNumberForDisplay(number: String): String {
        ...
        } else if (number.length == LENGTH_AMERICAN_EXPRESS) {
            return formatAmericanExpressForDisplay(number)
        }
    ...
    private fun formatAmericanExpressForDisplay(number: String): String {
        val result = StringBuilder()
        for (idx in number.indices) {
            if (idx == 4 || idx == 10) {
                result.append(" ")
